...
namespace Identity.API.Services
{
    public class RoleService : IRoleService
    {
        ...
        public async Task<ApiResponse<int>> DeleteAsync(int id)
        {
            ...
            if (role == null)
            {
                return new ApiResponse<int>(404, 0, "Không tìm thấy vai trò cần xóa");
            }
            if (role.RoleName == "Admin" || role.RoleName == "User")
            {
                return new ApiResponse<int>(400, 0, "Không thể xóa quyền Admin hay User");
            }
            ...
            if (result > 0)
            {
                _logger.Information($"End : RoleService - DeleteAsync : {id} - Xóa thất bại");
                return new ApiResponse<int>(200, result, "Xóa vai trò thành công");
            }
            else
            {
                _logger.Information($"End : RoleService - DeleteAsync : {id} - Xóa thất bại");
                return new ApiResponse<int>(400, result, "Xóa vai trò thất bại");
            }
        }
        ...
        public async Task<ApiResponse<List<RoleResponseDTO>>> GetAllAsync()
        {
            ...
            _logger.Information($"Mapping list of role to dto");
            var data = _mapper.Map<List<RoleResponseDTO>>(roles);
            _logger.Information($"End : RoleService - GetAllAsync");
            return new ApiResponse<List<RoleResponseDTO>>(200, data, "Lấy dữ liệu thành công");
        }
        ...
        public async Task<ApiResponse<RoleResponseDTO>> GetByIdAsync(int id)
        {
            ...
            if (role == null)
            {
                return new ApiResponse<RoleResponseDTO>(404, null, "Không tìm thấy vai trò");
            }
            var data = _mapper.Map<RoleResponseDTO>(role);
            _logger.Information($"End : RoleService - GetByIdAsync");
            return new ApiResponse<RoleResponseDTO>(200, data, "Lấy dữ liệu vai trò thành công");
        }

        public async Task<ApiResponse<int>> InsertAsync(RoleRequestDTO item)
        {
            ...
            {
                return new ApiResponse<int>(400, -1, "Tên vai trò đã tồn tại");
            }
            var roleEntity = _mapper.Map<Role>(item);
            var newId = await _roleRepository.CreateAsync(roleEntity);
            _logger.Information($"End : RoleService - CreateAsync");
        ...
        public async Task<ApiResponse<RoleResponseDTO>> UpdateAsync(RoleRequestDTO item)
        {
            ...
            {
                return new ApiResponse<RoleResponseDTO>(400, null, "Tên vai trò đã tồn tại");
            }
            role = _mapper.Map<Role>(item);
            var result = await _roleRepository.UpdateAsync(role);
            if (result > 0)
            {
                return new ApiResponse<RoleResponseDTO>(200, _mapper.Map<RoleResponseDTO>(role), "Cập nhật thành công");
            }
            _logger.Information($"End : RoleService - UpdateAsync");
            return new ApiResponse<RoleResponseDTO>(200, null, "Có lỗi xảy ra");
        }
        ...
        public async Task<ApiResponse<RoleDetailDTO>> UpdateRoleDetailByRoleIdAsync(string roleId,RoleDetailDTO item)
        {
            ...
            return new ApiResponse<RoleDetailDTO>(200, _mapper.Map<RoleDetailDTO>(data),"Cập nhật thành công");
        }
    }
}
