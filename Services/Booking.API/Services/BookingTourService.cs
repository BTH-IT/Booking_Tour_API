...
namespace Booking.API.Services
{
	public class BookingTourService : IBookingTourService
	{
		...
		public async Task<ApiResponse<BookingTourCustomResponseDTO>> GetByIdAsync(int id)
		{
			...
			try
			{
				var cacheKey = $"BookingTour_{id}";
				var cachedData = await _cache.GetStringAsync(cacheKey);
				if (!string.IsNullOrEmpty(cachedData))
				{
					var cachedResponse = JsonSerializer.Deserialize<BookingTourCustomResponseDTO>(cachedData);

					_logger.Information("End: BookingTourService - GetAllAsync");
					return new ApiResponse<BookingTourCustomResponseDTO>(200, cachedResponse, "Data retrieved successfully", true);
				}
		...
		public async Task<ApiResponse<BookingTourCustomResponseDTO>> UpdateBookingTourInfoAsync(int bookingTourId, UpdateBookingTourInfoRequest request, int userId, int role)
		{
			try
			{
				...
				// Invalidate cache
				await _cache.RemoveAsync($"BookingTour_{bookingTourId}");
				await _cache.RemoveAsync($"BookingTour_User_{userId}");
		...
		public async Task<ApiResponse<List<BookingTourCustomResponseDTO>>> GetCurrentUserAsync(int userId)
		{
			...
			try
			{
				var cacheKey = $"BookingTour_User_{userId}";
				var cachedData = await _cache.GetStringAsync(cacheKey);
				if (!string.IsNullOrEmpty(cachedData))
				{
					var cachedResponse = JsonSerializer.Deserialize<List<BookingTourCustomResponseDTO>>(cachedData);

					_logger.Information("End: BookingTourService - GetAllAsync");
					return new ApiResponse<List<BookingTourCustomResponseDTO>>(200, cachedResponse, "Data retrieved successfully", true);
				}
				...
				await _cache.SetStringAsync(cacheKey, JsonSerializer.Serialize(bookingTourDtos), cacheOptions);
		...
		public async Task<ApiResponse<string>> UpdateStatusBookingTourAsync(int bookingTourId, UpdateBookingStatusDTO dto)
		{
			...
			// Invalidate cache
			await _cache.RemoveAsync($"BookingTour_{bookingTourId}");
			await _cache.RemoveAsync($"BookingTour_User_{bookingTour.UserId}");
